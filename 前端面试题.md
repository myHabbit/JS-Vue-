...
给**父元素**注册事件 当我们触发子元素的时候 会冒泡到父元素身上 从而触发父元素的事件  state: () => {
...
​    CatgoryList(state, payload) {

​      state.catgory = payload
...
call 和 apply 会立即执行函数，并且能够指定函数执行时的上下文（this 值）以及参数。

bind 返回一个新函数，不会立即执行，允许你在稍后的时候调用执行，并且永久绑定了执行时的上下文和预设参数
...
const boundFunc = greet.bind(obj, 'Hey');
...
Promise（承诺）是 JavaScript 中处理异步操作的一种机制。它代表了一个尚未完成但最终会完成的操作，并提供了处理操作完成或失败情况的方法
...
通过 Promise，我们可以更好地处理异步操作，避免了回调地狱问题，并以一种更清晰、可读性更高的方式编写异步代码。
...
- 挂起（Pending）：初始状态，表示异步操作正在进行中，尚未完成。
- 已完成（Fulfilled）：表示异步操作成功完成，并返回了一个值。
- 已拒绝（Rejected）：表示异步操作失败，并返回了一个错误原因。

结果：当 Promise 完成时，它会传递一个值作为异步操作的结果。这个结果可以是任何 JavaScript 数据类型，如字符串、对象、数组等。

处理异步结果：通过注册回调函数来处理 Promise 的完成或拒绝情况。可以使用 `then()` 方法注册成功回调函数，使用 `catch()` 方法注册失败回调函数。

链式调用：Promise 具有链式调用的能力，可以将多个操作串联在一起，依次处理异步操作的结果。这种链式调用可以提高代码的可读性和维护性。
...
```js
如果回调函数返回一个值（非 Promise 对象），那么新的 Promise 对象将被解析为已完成（fulfilled）状态，并以该值作为成功值。
如果回调函数返回一个 Promise 对象，那么新的 Promise 对象将以该 Promise 对象的状态和值来决定自己的状态
如果回调函数抛出一个异常，那么新的 Promise 对象将被拒绝（rejected），并以该异常作为拒因。
```
...
通过使用 `async` 和 `await`，我们可以以一种更直观和顺序的方式编写异步代码，避免了回调地狱问题，并使代码更易于阅读和维护。
...
`await` 关键字只能在 `async` 函数内部使用，它可以暂停函数的执行，等待 Promise 对象的完成，然后获取 Promise 对象的结果。同时，`await` 关键字可以将异步操作的结果直接赋值给一个变量，使得代码更加简洁和易读。
...
```js
push()         向数组末尾添加元素 并返回修改后的数组的新长度。
unshift()      向数组开头添加元素 并返回修改后的数组的新长度。
pop()          删除数组最后一个元素
shift()        删除数组第一个元素
concat()：     将两个或多个数组合并成一个新数组。
slice()：      截取数组的一部分，返回截取后的新数组。array.slice(1, 4); !注意 左边包括右边不包括 右边不包括该索引对应的元素
join()：       将数组中的所有元素以指定的分隔符连接成一个字符串。
map()：        根据原数组的每个元素，生成一个新数组，新数组的元素由回调函数的返回值决定。 方法返回一个新数组，
forEach()：    遍历数组的每个元素，并对每个元素执行指定的操作    方法没有返回值，它只是遍历数组并对每个元素执行指定的操作。
sort()：       对数组进行排序，可以指定排序函数。
reverse()：    将数组中的元素顺序颠倒。
indexOf()      从数组开头查询元素 如果查询到返回数组下标 查询不到返回-1
lastIndexOf()  从数组末尾开始查询元素 如果查询到返回数组下标 查询不到返回-1
find()         传递一个回调函数 接收一个参数 item 为数组每一项  返回值为符合条件的哪一项
findIndex()    传递一个回调函数 接收一个参数 item 为数组每一项  返回值为符合条件的哪一项的索引
every()        传递一个回调函数 接收一个参数 item 为数组每一项  当所有的元素中的某一项的值为true时 才会返回true
reduce()       用于做累加  方法接收两个参数：一个回调函数和一个可选的初始值 回调函数接收两个参数 第一项为每次处理后的值 第二项为数组每一项  返回值为最终累计之后的结果 arr.reduce((a, c) => { return a + c.count * c.price;} , 0);
filter()       过滤器 该方法 传递一个回调函数 接收一个参数 item  为数组每一项  filter 方法不会修改原始数组，而是返回一个新的数组，该数组为符合条件的元素 
```
...
```js
query（查询参数）传值：使用 Query 参数作为 URL 的一部分来传递参数。Query 参数以 ? 开头，多个参数之间用 & 分隔，例如 http://example.com/path?param1=value1&param2=value2。在 Vue 路由中，可以使用 $route.query 或 $router.push 方法来获取和设置 Query 参数。

示例：$route.query.param1 可以获取到上述 URL 中的 param1 的值。
优点：

参数直接显示在 URL 中，便于查看、分享和导航。
可以传递多个参数，参数之间相对灵活。
缺点：
参数对用户可见，可能存在安全性风险。
URL 难以阅读和记忆，特别是有多个参数时。

Params（路径参数）传值：使用 Params 参数将参数直接嵌入到 URL 路径中。Params 参数以冒号 : 开头，例如 /path/:param1/:param2。在 Vue 路由中，可以使用 $route.params 或 $router.push 方法来获取和设置 Params 参数。

示例：$route.params.param1 可以获取到 /path/value1/value2 中的 value1。
优点：

参数不对用户可见，更安全。
URL 更加简洁和易读，特别是在只有少量参数时。
缺点：

只能传递固定数量的参数，且参数顺序需要保持一致。
不适合传递较多的参数。
总结：Query 传值适合传递少量、对用户可见的参数，而 Params 传值适合传递固定数量的参数且不对用户可见。选择使用哪种方式取决于具体的场景和需求。
```
...
```js
1.当初次渲染 xxx 一开始是一个空对象 尝试访问一个空对象下面的数据 是个underfind 访问underfind[0]没有东西所以会报错
 解决办法
   <!-- 
         1.可选链的语法?.
       <el-breadcrumb-item
          :to="{ path: `/catgory/${detailList.categories?.[1].id}` }"
