...
pedding  resovle  reject

pedding  表示等待中 还没有结果

resolved  已经完成 表示得到了我们想要结果 可以继续往下执行
...
路由传值分为两种：
 ...
 !!!注意点1
   	params 传参后，刷新页面会失去拿到的参数。所以路由参数要修改为 '/login/:username'（官方称为动态路	由）
...
import vueResource  from 'vue-resourse'
...
2。子组件用:v-bind将数据传到插槽的使用者

3.使用者结构外侧必须报过一个template 并且加一个scope属性 scope=“atguigu”，此处的atguigu是个对象



### vuex

Vuex 是一个专门在vue中实现集中式状态管理库。它解决了在大型单页应用程序中，组件间数据传递和状态管理的问题。
...
Actions Mutations State 三个的数据类型都为对象  必须用store调用
